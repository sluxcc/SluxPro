::https://tinyurl.com/sluxwarelicense
@echo off
Mode 52,16
title SluxPro
set Version=9.4
...
::Begin Log
set error="%tmp%\SluxProError.txt"
set log="%tmp%\SluxProLog.txt"
...
:DarkMode
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v DarkMode 2^>nul') do set /a "DarkMode=%%a"
if "%DarkMode%" equ "1" (
::Text Color Red
set col1=[91m
...
if %errorlevel% neq 0 (
call:SluxProLogo
echo              %col2%Run SluxPro as Admin%col1%
powershell -NoProfile -NonInteractive -Command start -verb runas "'%~s0'" >nul 2>&1
if !errorlevel! equ 0 exit /b
call:SluxProLogo
echo.
echo            %col2%SluxPro is not running as Admin!
...
::Check For Updates
curl -g -k -L -# -o "%tmp%\latestVersion.bat" "https://github.com/sluxcc/SluxPro/tree/main/Files/Version" >nul 2>&1
call "%tmp%\latestVersion.bat"
if "%DevBuild%" neq "Yes" if "%Version%" lss "!latestVersion!" (cls
	call:SluxProLogo
	echo.
	echo             Warning, SluxPro isn't updated.
	echo        Would you like to update to version %col2%!latestVersion!?
	echo.
	choice /c:"YN" /n /m "%BS%                   [Y] Yes  [N] No"
	if !errorlevel! equ 1 (
		curl -L -o "%~s0" "https://github.com/sluxcc/SluxPro/tree/main/Files/" >nul 2>&1
		call "%~s0"
	)
)
...
::WMI Settings
Reg add "HKCU\Software\SluxPro" /f >nul 2>&1
...
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v CORES') do set CORES=%%a
for /f "tokens=*" %%a in ('Reg query "HKCU\Software\SluxPro" /v GPU_NAME') do set GPU_NAME=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v mem') do set mem=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v ChassisTypes') do set ChassisTypes=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Degrees') do set Degrees=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v osarchitecture') do set osarchitecture=%%a
) >nul 2>&1 else (
...
curl -g -k -L -# -o "%tmp%\NSudo.exe" "https://github.com/sluxcc/SluxPro/tree/main/Files/NSudo.exe" >nul 2>&1
)
...
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Restore') do set Restore=%%a) >nul 2>&1
if "%Restore%" equ "" (cls
call:SluxProLogo
echo.
echo           Let SluxPro create a Restore Point
echo              (Used to undo all changes^)
choice /c:NY /n /m "%BS%                  [Y] Yes  [N] No"
Reg add "HKCU\Software\SluxPro" /v Restore /t REG_DWORD /d "!errorlevel!" /f >nul
)

::Slider
for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\SluxPro /v opt 2^>nul') do set /a opt=%%a
...
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Throttling 2^>nul') do set "Throttling=%%a"
if "!Throttling!" equ "" Reg add "HKCU\Software\SluxPro" /v Throttling /t REG_DWORD /d "0" /f >nul
) else (
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v PowMax 2^>nul') do set "PowMax=%%a"
if "!PowMax!" equ "" Reg add "HKCU\Software\SluxPro" /v PowMax /t REG_DWORD /d "1" /f >nul
)

for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v NVCP 2^>nul') do set "NVCP=%%a"
for %%a in (391.35 425.31 441.XX 461.72 456.71 457.30 461.72 461.92 466.11) do (
if not defined NVCP if "%NvidiaDriverVersion%" equ "%%a" Reg add "HKCU\Software\SluxPro" /v NVCP /t REG_DWORD /d "1" /f >nul
)
...
cls
if "%SettingsPage%"=="1" (
for %%i in (Idle sleepstates PowMax Throttling) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\SluxPro /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 1/4
...
if %SettingsItem% equ 1 if "%Idle%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v Idle /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v Idle /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 2 if "%sleepstates%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v sleepstates /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v sleepstates /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%PowMax%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v PowMax /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v PowMax /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%Throttling%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v Throttling /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v Throttling /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% lss 5 goto Settings

if "%SettingsPage%"=="2" (
for %%i in (cstates pstates KBoost NVCP) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\SluxPro /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 2/4
...
if %SettingsItem% equ 1 if "%cstates%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v cstates /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v cstates /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 2 if "%pstates%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v pstates /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v pstates /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%KBoost%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v KBoost /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v KBoost /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%NVCP%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v NVCP /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v NVCP /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% lss 5 goto Settings

if "%SettingsPage%"=="3" (
for %%i in (Res DSCP staticip Mouse) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\SluxPro /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 3/4
...
if %SettingsItem% equ 1 if "%staticip%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v staticip /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v staticip /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 2 if "%DSCP%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v DSCP /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v DSCP /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%Res%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v Res /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v Res /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%Mouse%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v Mouse /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v Mouse /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 5 goto Settings
if %SettingsItem% lss 5 goto Settings

if "%SettingsPage%"=="4" (
for %%i in (DarkMode Restore DisplayScaling honepow) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\SluxPro /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 4/4
echo  %col1%[%col2%1%col1%] SluxPro Light Mode !DarkMode!
echo  [90mSwitch the SluxPro GUI into light mode
echo.
...
if %SettingsItem% equ 1 if "%DarkMode%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v DarkMode /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v DarkMode /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 1 call :DarkMode "change"
if %SettingsItem% equ 2 if "%Restore%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v Restore /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v Restore /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%DisplayScaling%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v DisplayScaling /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v DisplayScaling /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%honepow%"=="[32mon" (Reg add "HKCU\Software\SluxPro" /v honepow /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\SluxPro" /v honepow /t REG_DWORD /d "1" /f >nul)
...
curl -g -k -L -# -o "%tmp%\nvidiaProfileInspector\SluxProProfile.nip" "https://github.com/sluxcc/SluxPro/tree/main/Files/SluxProProfile.nip"
)

if "%Res%"=="0x1" if not exist "%SystemDrive%\EchoRes.exe" (
echo Downloading Timer Resolution [...]
curl -g -k -L -# -o "%SystemDrive%\EchoRes.exe" "https://github.com/sluxcc/SluxPro/tree/main/Files/SetTimerResolutionService.exe" >nul 2>&1
)
...
::Disable FTH
Reg add "HKLM\Software\Microsoft\FTH" /v "Enabled" /t REG_DWORD /d "0" /f >>%log% 2>>%error%
echo Disable FTH
...
Reg add "HKLM\Software\Microsoft\Windows NT\CurrentVersion\Winlogon" /v "AutoRestartShell" /t REG_DWORD /d "1" /f >>%log% 2>>%error%
...
Reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\Display" /v "EnableGdiDPIScaling" /t REG_DWORD /d "1" /f >>%log% 2>nul
...
if "%Idle%" equ "0x1" (
set SluxProPowName=%SluxProPowName% DIdle
::Disable C-States
...
set SluxProPowName=%SluxProPowName% dsleep
...
if "%PowMax%" equ "0x1" (
set SluxPorPowName=%SluxProPowName% MAX
...
::Apply
if "%honepow%" neq "0x1" powercfg -changename scheme_current "%SluxProPowName%" "For SluxPro Optimizer %Version% (dsc.gg/Slux.cc) By Slux" >>%log% 2>>%error%
...
::Netsh
netsh int tcp set supplemental template=Internet >>%log% 2>>%error%
netsh int tcp set supplemental Internet congestionprovider=bbr2 enablecwndrestart=disable >>%log% 2>>%error%
...
for %%F in ("%file%") do Reg query "HKCU\Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers" /v "%file%" >nul 2>&1 && (
	...
	echo Undo GPU High Performance
	echo Undo Disable Fullscreen Optimizations
	echo Undo Run As Administrator
	echo Undo CPU High Class
) || (
...
if not exist "%tmp%\devmanview.exe" curl -g -L -# -o "%tmp%\devmanview.exe" "https://github.com/sluxcc/SluxPro/tree/main/Files/DevManView.exe"
echo Downloading EmptyStandbyList [...]
if not exist "%tmp%\EmptyStandbyList.exe" curl -g -L -# -o "%tmp%\EmptyStandbyList.exe" "https://github.com/notslux/SluxPro"
cls
...
Reg add HKCU\Software\SluxPro /v PowMax /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v Throttling /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v Idle /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v KBoost /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v Res /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v cstates /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v pstates /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v sleepstates /t REG_DWORD /d 0 /f
) >nul
if "%opt%" equ "2" (set opt2=[33m
Reg add HKCU\Software\SluxPro /v PowMax /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v Throttling /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v Idle /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v KBoost /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v Res /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v cstates /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v pstates /t REG_DWORD /d 0 /f
Reg add HKCU\Software\SluxPro /v sleepstates /t REG_DWORD /d 1 /f
) >nul
if "%opt%" equ "3" (set opt3=[31m
Reg add HKCU\Software\SluxPro /v PowMax /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v Throttling /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v Idle /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v KBoost /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v Res /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v cstates /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v pstates /t REG_DWORD /d 1 /f
Reg add HKCU\Software\SluxPro /v sleepstates /t REG_DWORD /d 1 /f
echo msgbox "Warning^! The performance preset will display your CPU at 100%% usage in the task manager^." >"%tmp%\tmp.vbs"
Reg query HKCU\Software\SluxPro /v performancemode 2>nul || wscript "%tmp%\tmp.vbs"
Reg add HKCU\Software\SluxPro /v performancemode /t REG_DWORD /d 1 /f
) >nul
cls
echo.
echo       %col1%Speed up %col2%performance%col1%, %col2%latency%col1%, and %col2%ping%col1%
...
Reg add "HKCU\Software\SluxPro" /v opt /t REG_DWORD /d %errorlevel% /f >nul
goto Slider
...
echo                     SluxWare
echo.
echo            Discord Tag: vaxtr#0
echo.
echo                  SluxPro Discord Server
echo        %col2% https://discord.com/invite/adxFhBt7PU %col1%
echo.
echo                     Youtube Channel
echo  %col2% www.youtube.com/channel/UCSxVtnB16a-h2QuH1wqt1dw %col1%
echo.
echo [===============================================P1=]
...
:GrabSettings
::Power
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v PowMax') do set PowMax=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Throttling') do set Throttling=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Idle') do set Idle=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v NVCP') do set NVCP=%%a) >nul 2>&1
::Advanced
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Debloat') do set Debloat=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v DisplayScaling') do set DisplayScaling=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v KBoost') do set KBoost=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Restore') do set Restore=%%a) >nul 2>&1
::Optional
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v staticip') do set staticip=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Res') do set Res=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v Mouse') do set Mouse=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v DSCP') do set DSCP=%%a) >nul 2>&1
::Power2
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v cstates') do set cstates=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v pstates') do set pstates=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v honepow') do set honepow=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\SluxPro" /v sleepstates') do set sleepstates=%%a) >nul 2>&1
goto:eof
